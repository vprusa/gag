...
public class LineData {
    ...
    public LineData(Date date, float[] quat, float[] acc, Sensor sensor, Hand hand) {
        ...
        if (acc == null || acc.length < 3) {
            this.acc = new float[] { 0.0f, 0.0f, 0.0f };
        } else {
            this.acc = acc;
        }
        this.hand = hand;
    ...
    public String toFileString() {
        // String reportData = date != null ? Configuration.dfDate.format(date) : "" + "
        // " + (hand == Hand.LEFT ? "*" : "$") + " "
        // + sensor.ordinal() + " " + quat[0] + " " + quat[1] + " " + quat[2] + " " +
        // quat[3] + acc != null ? (" " + acc[0]
        // + " " + acc[1] + " " + acc[2] ) : "";

        String reportData = (date != null ? Configuration.dfDate.format(date) : "") + " "
                + (hand == Hand.LEFT ? "*" : "$") + " " + sensor.ordinal() + " " + quat[0] + " " + quat[1] + " "
                + quat[2] + " " + quat[3] + " " + acc[0] + " " + acc[1] + " " + acc[2];
